...
void sleep_random(int minimumTime_ms = 100, int maximumTime_ms = 500) {
...
void* philosopherFunction(void* v_arg) {
    ...
    for(unsigned long long i = 0; i < rounds; i++) {
        cout << "[Philosopher " << index << "] is thinking (round " << (i + 1) << ")\n";
        ...
        cout << "[Philosopher " << index << "] has picked up forks (" << leftIndex << ", " << rightIndex << "), eating...\n";
        sleep_random(200, 600);

        pthread_mutex_unlock(&forks[firstFork]);
        pthread_mutex_unlock(&forks[secondFork]);
        cout << "[Philosopher " << index << "] has put down forks (" << leftIndex << ", " << rightIndex << "), thus round " << (i + 1) << " is done.\n";
    }

    cout << "[Philosopher " << index << "] Finished all " << rounds << " rounds.\n";
    return nullptr;
}
